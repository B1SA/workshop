sap.ui.define([
	...
	return BaseObject.extend("xxx.model.GroupSortState", {
		 ...
		 * @alias xxx.model.GroupSortState
		 */
		...
		/**
		 * Sorts by CardName, or by Balance
		 *
		...
		/**
		 * Groups by Balance, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Balance") {
				...
				this._oViewModel.setProperty("/sortBy", "Balance");

				aSorters.push(
					new Sorter("Balance", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "CardName");
			}
